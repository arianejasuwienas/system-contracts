...
// the generated contract is using hedera response codes from services version 0.68.0-SNAPSHOT
// https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/response_code.proto
...
library HederaResponseCodes {
    ...
    int32 internal constant BATCH_LIST_EMPTY = 388;

    /// The list of batch transactions contains duplicated transactions
    int32 internal constant BATCH_LIST_CONTAINS_DUPLICATES = 389;
    ...
    int32 internal constant BATCH_TRANSACTION_IN_BLACKLIST = 390;

    /// The inner transaction of a batch transaction failed
    int32 internal constant INNER_TRANSACTION_FAILED = 391;

    /// The inner transaction of a batch transaction is missing a batch key
    int32 internal constant MISSING_BATCH_KEY = 392;

    /// The batch key is set for a non batch transaction
    int32 internal constant BATCH_KEY_SET_ON_NON_INNER_TRANSACTION = 393;

    /// The batch key is not valid
    int32 internal constant INVALID_BATCH_KEY = 394;

    /// The provided schedule expiry time is not configurable.
    int32 internal constant SCHEDULE_EXPIRY_NOT_CONFIGURABLE = 395;

    /// The network just started at genesis and is creating system entities.
    int32 internal constant CREATING_SYSTEM_ENTITIES = 396;

    /// The least common multiple of the throttle group's milliOpsPerSec is too large and it's overflowing.
    int32 internal constant THROTTLE_GROUP_LCM_OVERFLOW = 397;

    /// Token airdrop transactions can not contain multiple senders for a single token.
    int32 internal constant AIRDROP_CONTAINS_MULTIPLE_SENDERS_FOR_A_TOKEN = 398;

    /// The GRPC proxy endpoint is set in the NodeCreate or NodeUpdate transaction, which the network does not support.
    int32 internal constant GRPC_WEB_PROXY_NOT_SUPPORTED = 399;

    /// An NFT transfers list referenced a token type other than NON_FUNGIBLE_UNIQUE.
    int32 internal constant NFT_TRANSFERS_ONLY_ALLOWED_FOR_NON_FUNGIBLE_UNIQUE = 400;

    /// A HAPI client cannot set the SignedTransaction#use_serialized_tx_message_hash_algorithm field.
    int32 internal constant INVALID_SERIALIZED_TX_MESSAGE_HASH_ALGORITHM = 401;

    /// An EVM hook execution was throttled due to high network gas utilization.
    int32 internal constant EVM_HOOK_GAS_THROTTLED = 500;

    /// A user tried to create a hook with an id already in use.
    int32 internal constant HOOK_ID_IN_USE = 501;

    /// A transaction tried to execute a hook that did not match the specified type or was malformed in some other way.
    int32 internal constant BAD_HOOK_REQUEST = 502;

    /// A CryptoTransfer relying on a ACCOUNT_ALLOWANCE hook was rejected.
    int32 internal constant REJECTED_BY_ACCOUNT_ALLOWANCE_HOOK = 503;

    /// A hook id was not found.
    int32 internal constant HOOK_NOT_FOUND = 504;

    /// A lambda mapping slot, storage key, or storage value exceeded 32 bytes.
    int32 internal constant LAMBDA_STORAGE_UPDATE_BYTES_TOO_LONG = 505;

    /// A lambda mapping slot, storage key, or storage value failed to use the minimal representation (i.e., no leading zeros).
    int32 internal constant LAMBDA_STORAGE_UPDATE_BYTES_MUST_USE_MINIMAL_REPRESENTATION = 506;

    /// A hook id was invalid.
    int32 internal constant INVALID_HOOK_ID = 507;

    /// A lambda storage update had no contents.
    int32 internal constant EMPTY_LAMBDA_STORAGE_UPDATE = 508;

    /// A user repeated the same hook id in a creation details list.
    int32 internal constant HOOK_ID_REPEATED_IN_CREATION_DETAILS = 509;

    /// Hooks are not not enabled on the target Hiero network.
    int32 internal constant HOOKS_NOT_ENABLED = 510;

    /// The target hook is not a lambda.
    int32 internal constant HOOK_IS_NOT_A_LAMBDA = 511;

    /// A hook was deleted.
    int32 internal constant HOOK_DELETED = 512;

    /// The LambdaSStore tried to update too many storage slots in a single transaction.
    int32 internal constant TOO_MANY_LAMBDA_STORAGE_UPDATES = 513;

    /// A lambda mapping slot, storage key, or storage value failed to use the minimal representation (i.e., no leading zeros).
    int32 internal constant HOOK_CREATION_BYTES_MUST_USE_MINIMAL_REPRESENTATION = 514;

    /// A lambda mapping slot, storage key, or storage value exceeded 32 bytes.
    int32 internal constant HOOK_CREATION_BYTES_TOO_LONG = 515;

    /// A hook creation spec was not found.
    int32 internal constant INVALID_HOOK_CREATION_SPEC = 516;

    /// A hook extension point was empty.
    int32 internal constant HOOK_EXTENSION_EMPTY = 517;

    /// A hook admin key was invalid.
    int32 internal constant INVALID_HOOK_ADMIN_KEY = 518;

    /// The hook deletion requires the hook to have zero storage slots.
    int32 internal constant HOOK_DELETION_REQUIRES_ZERO_STORAGE_SLOTS = 519;

    /// Cannot set both a hook call and an approval on the same AccountAmount or NftTransfer message.
    int32 internal constant CANNOT_SET_HOOKS_AND_APPROVAL = 520;

    /// The attempted operation is invalid until all the target entity's hooks have been deleted.
    int32 internal constant TRANSACTION_REQUIRES_ZERO_HOOKS = 521;

    /// The HookCall set in the transaction is invalid
    int32 internal constant INVALID_HOOK_CALL = 522;

    /// Hooks are not supported to be used in TokenAirdrop transactions
    int32 internal constant HOOKS_ARE_NOT_SUPPORTED_IN_AIRDROPS = 523;
    ...
    int32 internal constant ACCOUNT_IS_LINKED_TO_A_NODE = 524;
    ...
    int32 internal constant HOOKS_EXECUTIONS_REQUIRE_TOP_LEVEL_CRYPTO_TRANSFER = 525;
    ...
    int32 internal constant NODE_ACCOUNT_HAS_ZERO_BALANCE = 526;
